...
local myModels = models
for path in (...):gmatch("[^/.]+") do
   myModels = myModels[path]
end
---@type ModelPart
myModels = myModels.model
myModels:remove()
...
function Action:setIconTexture(texture, pos, size)
   local model = iconModelTemplate:copy("")
...
local function makeActionPopup(action)
   ...
   for _, v in pairs(myModels.slider:getChildren()) do
      v:copy(v:getName())
         :light(15, 15)
         :moveTo(model)
   end
